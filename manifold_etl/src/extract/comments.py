"""Extraction helpers for Manifold comments."""
...
def fetch_comments_for_user(
    ...
    *,
    ...
    """Return all comments authored by ``user_id``."""
    per_page = page_limit or config.COMMENT_PAGE_LIMIT
    if per_page <= 0:
    ...
    while True:
        ...
        try:
            ...
            logger.warning(
                "Failed to fetch comments for user %s: %s",
                user_id,
            ...
            logger.warning("Request exception while fetching comments for %s: %s", user_id, exc)
            break
        except Exception as exc:  # pragma: no cover - defensive
            logger.exception("Unexpected error while fetching comments for %s: %s", user_id, exc)
            break
